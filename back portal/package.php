...
if(!company == ''){ 
var url ="package.php?company="+company;
...
var url = "package.php?company="+company_sel+"&department="+department;
}
...
var url ="package.php?new_value_test="+new_value;
}
$(location).attr('href',url);
...
$.ajax({
 ...
 $verification=$_POST['verification'];
  ...
  // echo "Please fill all the required field";
   ?>
    <div style="text-align:center; color:red;"> Please fill the Package no.  </div>
    <?php
   }
   else
 {
 ...
 $number1=mysqli_num_rows($res50);
   ...
   if($number1==0)
   {
     $insert=mysqli_query($con,"INSERT INTO patient_user(verification_code)
VALUES ('".$verification."')");
...
<p><span>Package No.<span style="color:red;">*</span>: </span> <span style="margin-left:25px;"> <input type="text" name="verification"> </span> </p>
...
<?php  //echo '<td><a href="../admin/package.php?delete='.$id.'" title="Delete" style="color:red;"><i class="icon-remove"></i></a></td>'; 
 ?>
<?php  echo '<td><i class="icon-remove" data-datac="'.$id.'" style="color:#f00;cursor:pointer;" ></i></td>';  ?>
...
$baseurl='package.php?company='.$fcompany;
}
else if(!$fcompany=='' && !$fdepartment==''){
$baseurl='package.php?company='.$fcompany.'&department='.$fdepartment;
}
else if(!$key ==''){
$baseurl='package.php?key='.$key;
}
else{
$baseurl='package.php?';
}
